...
UPDATE public.pos_ventas AS venta
SET sucursal_id = caja.sucursal_id
FROM public.pos_cajas AS caja
WHERE venta.caja_id = caja.id
  AND venta.sucursal_id IS NULL;
...
WHERE sucursal_id IS NULL
  AND (SELECT count(*) FROM public.pos_sucursales WHERE activa = true) = 1;

CREATE INDEX IF NOT EXISTS pos_ventas_sucursal_idx ON public.pos_ventas (sucursal_id);
...
BEGIN
  ...
  IF v_sucursal_id IS NOT NULL AND venta_sucursal_pendiente(v_venta_id) THEN
    UPDATE public.pos_ventas SET sucursal_id = v_sucursal_id WHERE id = v_venta_id AND sucursal_id IS NULL;
  END IF;
